...
def build_PayeeDetails(transaction):
    ...
    if transaction['PayeeMinit'] == "":
        ...
        alias = f"{transaction['PayeeFName']} {transaction['PayeeMinit']} {transaction['PayeeLName']}"
...
def build_PaymentInfo(transaction):
    ...
    """
    PaymentInfo = {}
    PaymentInfo['PaymentIssuerBankParty'] = lookup(alias=transaction['BankName'], 
    alias_set='BankingInstitutions', lookup_file='BankParty.json')
    PaymentInfo['PaymentIssuerBankAccount'] = lookup(alias=transaction['CompanyBankAccount'],
...
def build_VLP(transaction):
    ...
    vlp['PaymentCompanyParty'] = lookup(alias=transaction['LegalEntity'], 
    alias_set='VenerableCompanies', lookup_file='OrganizationParty.json')
    vlp['PaymentCompanyGeneralLedgerAccount'] = lookup(alias='VIAC', 
    alias_set='VenerableAccounts', lookup_file='Account.json')
...
def build_ContextSource(transaction):
    ...
    ContextSource['TriggeringPaymentEventLocation'] = 'OFFICE' if transaction['SiteId'] == 'WC' else 'SITE'
    ContextSource['TriggeringPaymentEventID'] = transaction['Claim']
    ContextSource['TriggeringPaymentEventDatetime'] = transaction['SystemProcessDate'] #ask about this
